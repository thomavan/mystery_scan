...
PYBIND11_MODULE(zebra_scanner, m)
{
	...
	class_<Scanner>(m, "Scanner")
		...
		.def("scan_long_error", &Scanner::BeepLongError)
...
void Scanner::BeepLongError(string id)
{
    StatusID status;    
    std::string outXml;
	// beep
    std::string inXml = "<inArgs><scannerID>" + id + "</scannerID><cmdArgs><arg-int>10</arg-int></cmdArgs></inArgs>";
    ::ExecCommand(CMD_DEVICE_BEEP_CONTROL, inXml, outXml, &status);
